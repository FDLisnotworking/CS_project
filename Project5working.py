...
cursor=db.cursor()
#Fadil: Creating cursor
print(db)
#Fadil: Checking connection. Not required
print("[Insert Welcome text]")
#Fadil: Ignore this for now
while True:
    ...
    if c1=='1':
        ...
        if c2=='1':
            ...
            for i in range(n):
                print(i+1, ". ", sep='',end='')
                #Fadil: Above statement for neatness
                b_id=input("Enter book ID: ")
                cursor.execute("select book_name from books where book_id="+b_id)
                #Fadil: Fetching name of book using ID
                b_n=cursor.fetchone()[0]
                #Fadil: Saving book name to a variable
                cursor.execute('insert into sales values('+b_id+',"'+b_n+'",'+c_id+','+c_p+',"'+c_n+'")')
                db.commit()
                #Fadil: Inserting the values
            ...
            if c3=='1':
                ...
                #Fadil: Getting the name of book using book id. Same thing I did for adding record
                cursor.execute('update sales set book_id='+nb_id+', book_name="'+nb_n+'" where book_id='+b_id+' and customer_id='+c_id)
                #Fadil: Updating Values. Here I'm doing for both book ID and book name using only book ID
                db.commit()
                print("Record added sucessfully")
            elif c3=='2':
            ...
            if c3=='1':
                ...
                #Fadil: This part is to show what is being deleted
                L=cursor.fetchone()
                print("\nBook ID:", L[0])
                print("Book Name:", L[1])
                ...
                if c4=='y':
                    ...
                    db.commit()
                    #Fadil: db.commit() here is not a good idea (or anywhere else). For now I'm going to keep it here but will be removed later and added as a seperate option when the user is choosing. That way any mistakes can be undoed
            ...
            if c3=='1':
                ...
                if c4=='1':
                    ...
                    cursor.execute('select * from sales where book_id like "%'+c5+'%"')
                    #Fadil: Basically takes the value and uses the LIKE operator to find something similiar to what the user entered
                elif c4=='2':
